...
import com.remaistudio.master.adapter.ImageViewPagerAdapter;
import com.remaistudio.master.ads.admob.AdMobNativeAdvanceUnified;
import com.remaistudio.master.ads.admob.AdMobVideoRewarded;
import com.remaistudio.master.apps.builder.dialog.BuildingInstaller;
...
import com.remaistudio.master.architecture.viewmodel.OfferViewModel;
import com.remaistudio.master.architecture.viewmodel.PurchaseViewModel;
import com.remaistudio.master.architecture.viewmodel.RelatedItemViewModel;
...
import com.remaistudio.master.util.BlockLauncherHelper;
import com.remaistudio.master.util.ButtonColorManager;
import com.remaistudio.master.util.ColorList;
import com.remaistudio.master.util.ContentHelper;
import com.remaistudio.master.util.MinecraftHelper;
import com.remaistudio.master.util.ServerHelper;
...
import com.remaistudio.master.util.billing.PurchaseManager;
...
import com.remaistudio.master.util.screen.ScreenSize;
...
import static com.remaistudio.master.download.DownloadAsyncTask.STATUS_CANCELED;
import static com.remaistudio.master.download.DownloadAsyncTask.STATUS_MEMORY_ERROR;
import static com.remaistudio.master.download.DownloadAsyncTask.STATUS_NETWORK_ERROR;
import static com.remaistudio.master.download.DownloadAsyncTask.STATUS_SUCCESS;
import static com.remaistudio.master.util.BlockLauncherHelper.REQUEST_CODE_MOD;
import static com.remaistudio.master.util.BlockLauncherHelper.REQUEST_CODE_TEXTURE;
...
public class ActivityItem extends ActivityAppParent implements BuildingInstaller.BuildingListener,
    ...
    private TextView mTextViewRelatedMore;
    private TextView mTextViewOfferMore;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAdapterRelated = new AdapterRecyclerView(mRecycleViewRelated);
        mAdapterOffers = new AdapterRecyclerView(mRecyclerViewOffers);
        mRecycleViewRelated.setLayoutManager(RecyclerViewManager.LayoutManagerEnum.GridLayout);
        mRecycleViewRelated.setAdapter(mAdapterRelated);
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        mActivityItemViewModel.notifyViewModel();
        mOfferViewModel.notifyViewModel();
    }
...
//                                setButtonMainOpenMinecraftWithFile(event.file);
    ...
    private void setViewModel() {
        ...
        mActivityItemViewModel = ViewModelProviders.of(this, new RelatedItemViewModel.RelatedItemViewModelFactory(getApplication(), mItem)).get(RelatedItemViewModel.class);
        mActivityItemViewModel.getRelatedItemsLiveData().observe(this, this::setRecyclerViewRelatedItems);

        mOfferViewModel = OfferViewModel.get(this);
        mOfferViewModel.getItemLiveData().observe(this, this::setRecyclerViewOfferItems);
...
//                    description = descriptionTitle + description;
    ...
    private void setRecyclerViewRelatedItems(@Nullable List<JsonItemContent> items) {

        mTextViewRelatedMore.setOnClickListener(v -> {
            Intent intent = new Intent(ActivityItem.this, ActivityRelated.class);
            intent.putExtra(FRAGMENT_DATA, ContentHelper.getJsonNameItem(mItem));
        ...
        if (items != null && !items.isEmpty()) {
            ...
            itemList.subList(0, itemList.size() % ScreenSize.getColumnSize(SPAN_COUNT_DEFAULT, this)).clear();
            mAdapterRelated.setItemList(itemList);
            mRecycleViewRelated.removeAllViews();
            mRecycleViewRelated.setHasFixedSize(false);
            mRecycleViewRelated.setLayoutManager(RecyclerViewManager.LayoutManagerEnum.GridLayout, ScreenSize.getColumnSize(SPAN_COUNT_DEFAULT, this));
            mRecycleViewRelated.setAdapter(mAdapterRelated);
            mRecycleViewRelated.getAdapter().notifyDataSetChanged();
        } else {
            mRecycleViewRelated.setVisibility(View.GONE);
        }
    }

    private void setRecyclerViewOfferItems(@Nullable List<JsonItemContent> items) {

        mTextViewOfferMore.setOnClickListener(v -> {
//            ActivityItem.this.startActivity(new Intent(ActivityItem.this, ActivityOfferWall.class));
            overridePendingTransition(R.anim.enter, R.anim.exit);
        });
